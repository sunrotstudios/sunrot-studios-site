...
class ArcballControl {
  ...
  public orientation = quat.identity(quat.create());
  public pointerRotation = quat.identity(quat.create());
  ...
  private initialPointerPos = vec2.create();
  private _rotationVelocity = 0;
  private _combinedQuat = quat.identity(quat.create());
  private pointerDownTime = 0;
  private hasMoved = false;

  private readonly EPSILON = 0.1;
  private readonly IDENTITY_QUAT = quat.identity(quat.create());
  private readonly CLICK_THRESHOLD = 5; // pixels
  private readonly CLICK_TIME_THRESHOLD = 300; // milliseconds
  ...
  constructor(canvas: HTMLCanvasElement, updateCallback?: UpdateCallback) {
    ...
    // Use non-passive for pointerdown since we need preventDefault
    canvas.addEventListener("pointerdown", (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      const canvasX = e.clientX - rect.left;
      const canvasY = e.clientY - rect.top;
      console.log("🎯 InfiniteMenu: User started dragging at", canvasX, canvasY);
      vec2.set(this.pointerPos, canvasX, canvasY);
      vec2.copy(this.previousPointerPos, this.pointerPos);
      vec2.copy(this.initialPointerPos, this.pointerPos);
      this.isPointerDown = true;
      this.pointerDownTime = Date.now();
      this.hasMoved = false;
      // Prevent scrolling only when actively interacting with the 3D menu
      canvas.style.touchAction = "none";
      e.preventDefault();
      e.stopPropagation();
    }, { passive: false });

    // Use passive for pointerup since we don't prevent default
    canvas.addEventListener("pointerup", (e: PointerEvent) => {
      if (this.isPointerDown) {
        console.log("🎯 InfiniteMenu: User stopped dragging");
        const rect = canvas.getBoundingClientRect();
        const canvasX = e.clientX - rect.left;
        const canvasY = e.clientY - rect.top;
        const currentPos = vec2.fromValues(canvasX, canvasY);
        const distance = vec2.distance(this.initialPointerPos, currentPos);
        ...
        if (
          ...
          timeElapsed < this.CLICK_TIME_THRESHOLD &&
          !this.hasMoved
        ) {
          this.onItemClick?.();
        }
      ...
      // Re-enable scrolling when not interacting
      canvas.style.touchAction = "auto";
    }, { passive: true });

    // Use passive for pointerleave since we don't prevent default
    canvas.addEventListener("pointerleave", () => {
      if (this.isPointerDown) {
        console.log("🎯 InfiniteMenu: User left canvas area");
      }
      this.isPointerDown = false;
      // Re-enable scrolling when pointer leaves
      canvas.style.touchAction = "auto";
    }, { passive: true });

    // Use non-passive for pointermove since we need preventDefault during dragging
    canvas.addEventListener("pointermove", (e: PointerEvent) => {
      if (this.isPointerDown) {
        ...
        if (distance > this.CLICK_THRESHOLD && !this.hasMoved) {
          this.hasMoved = true;
          console.log("🎯 InfiniteMenu: User started significant movement");
        }

        vec2.set(this.pointerPos, canvasX, canvasY);
        // Prevent default behavior only when actively dragging
        e.preventDefault();
        e.stopPropagation();
      }
    }, { passive: false });

    // Use passive for wheel events since we want to allow scrolling
    canvas.addEventListener("wheel", (e: WheelEvent) => {
      if (this.isPointerDown) {
        // If actively dragging, prevent scroll and let the 3D menu handle it
        e.preventDefault();
      }
      // Otherwise, allow normal page scrolling
    }, { passive: false }); // Need non-passive to call preventDefault when dragging

    // Initially allow scrolling
    canvas.style.touchAction = "auto";
  }
  ...
  public update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    const oldOrientation = quat.clone(this.orientation);
    ...
    // Log significant orientation changes only during actual user interaction
    if (this.isPointerDown) {
      // Calculate quaternion difference using dot product
      const dot = Math.abs(quat.dot(oldOrientation, this.orientation));
      const orientationDifference = 1 - dot; // Distance measure for quaternions
      if (orientationDifference > 0.01) { // Only log significant changes during dragging
        console.log("🔄 ArcballControl: Rotating by", orientationDifference.toFixed(3));
      }
    }
...
class InfiniteGridMenu {
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.gl = gl;
    console.log("InfiniteGridMenu: WebGL2 context initialized successfully");
  ...
  private render(): void {
    if (!this.gl || !this.discProgram) {
      console.error("InfiniteGridMenu: Render failed - missing WebGL context or program");
      return;
    }
    const gl = this.gl;

    // Minimal rendering logging
    if (this._frames === 60) {
      console.log("✅ InfiniteGridMenu: Rendering pipeline active");
    }
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({
  ...
  // Notify parent of movement changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      const newActiveItem = items[itemIndex];
      setActiveItem(newActiveItem);
      // Also call the external callback immediately
  ...
  return (
    <div className="relative w-full h-full pointer-events-none">
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-full relative outline-none active:cursor-grabbing pointer-events-auto"
      />
        ...
        <div className="pointer-events-none">
          ...
          ${
            isMoving
              ...
              : "opacity-100 pointer-events-none duration-[500ms] translate-x-[-90%] -translate-y-1/2"
          }
        `}
          >
